def pumpit_clean(df: pd.core.frame.DataFrame):
    ...
    df["population"] = pd.cut(
        df["population"],
        [-1, 1, 25, 90, 160, 260, 9999999],
        labels=[0, 0.1, 0.2, 0.4, 0.6, 0.9],
    )
    ...
    df["construction_year"] = df.groupby(["region", "installer"])[
        "construction_year"
    ...
    df["quantity"] = df["quantity"].replace(
        {"enough": 1, "seasonal": 0.6, "insufficient": 0.4, "dry": 0, "unknown": 0}
    )
    ...
    df["waterpoint_type"] = df["waterpoint_type"].apply(
        lambda x: 1 if x in preferred_waterpoint else 0
    )
    ...
    # Encode 'payment' as binary: never pay = 0, else = 1
    df["extraction_type_class"] = np.where(
        df["extraction_type_class"] == "gravity", 0, 1
    )
